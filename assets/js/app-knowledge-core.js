...
(function (global) {
    ...
    class CoreKnowledgeComponent {
        constructor(hostId) {
            ...
            // Create Shadow DOM (Always use 'open' to allow presence check)
            if (!this._hostElement.shadowRoot) {
                this._shadow = this._hostElement.attachShadow({ mode: 'open' });
            } else {
                this._shadow = this._hostElement.shadowRoot;
        ...
        _renderList(container, searchTerm) {
            ...
            Object.entries(categories).forEach(([catKey, catLabel]) => {
                ...
                filteredItems.forEach(item => {
                    ...
                    // Tags HTML
                    const tagsHtml = (item.keywords || '').split(',').slice(0, 3).map(tag => 
                    ...
                    card.innerHTML = `
                        ...
                        <div class="card-body">
                            <p class="card-preview">${this._highlightText(item.desc.substring(0, 75), searchTerm)}...</p>
                            <div class="card-tags-row">${tagsHtml}</div>
                            <div class="card-expanded-content" style="display: none;">
                                <div class="expanded-inner">
                                    <p class="card-full-definition">${item.desc}</p>
                ...
                html += `
                    <div class="related-article-box">
                        ...
                        <div class="box-content">
                            <a href="../${item.link}" class="box-link">閱讀完整文章</a>
                        </div>
                    </div>
                `;
            } 

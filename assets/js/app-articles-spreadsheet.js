...
(function (global) {
    ...
    class ArticlesSpreadsheetComponent {
        ...
        loadData() {
            ...
            this._state.tracks = (window.TaxonomyConfig && typeof window.TaxonomyConfig.getAllTracks === 'function')
                ? window.TaxonomyConfig.getAllTracks()
                : (config.TRACKS || {});
            this._state.categories = config.CATEGORIES || {};
            this._state.tags = config.TAGS || {};
            this._state.series = config.SERIES || {};

            const getTrackObj = (id) => (window.TaxonomyConfig && window.TaxonomyConfig.getTrack ? window.TaxonomyConfig.getTrack(id) : (this._state.tracks[id] || null));
            ...
            repoPromise.then(repo => {
                ...
                const normalizedArticles = repo.map(item => {
                    ...
                    const trackObj = getTrackObj(item.trackId);
                    const trackName = trackObj ? trackObj.name : (item.trackId || '-');
        ...
        getPathPrefix() {
            ...
            if (segments.some(seg => subDirs.includes(seg))) {
                return '../';
            }
            return './';
        }
        ...
        readURLState() {
            ...
            if (track !== null && (track in this._state.tracks || track === 'all')) this._state.filter.track = track;
        ...
        syncURLState() {
            ...
            if (f.track !== 'all') params.set('track', f.track);
        ...
        filterData() {
            ...
            const { track, category, tag, series, year, keyword, query } = this._state.filter;
            ...
            this._state.filtered = this._state.articles.filter(item => {
                ...
                if (queryLower) {
                    ...
                    if (!title.includes(queryLower) && !desc.includes(queryLower) && !keywordsStr.includes(queryLower) && !summary.includes(queryLower) && !trackStr.includes(queryLower)) {
                        return false;
                    }
                }
                return true;
        ...
        exportTSV() {
            ...
            if (navigator.clipboard && navigator.clipboard.writeText) {
                ...
                this.showToast('瀏覽器不支援剪貼簿寫入，請手動反白表格複製。');
            }
        }
        ...
        exportMarkdown() {
            ...
            if (navigator.clipboard && navigator.clipboard.writeText) {
                ...
                this.showToast('瀏覽器不支援剪貼簿寫入，請手動選取表格。');
            }
        }
                ...
                <div class="spreadsheet-card bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden transition-colors duration-200">
                    ...
                    <div class="tv-filter-box p-4 bg-white border-b border-slate-200 space-y-3">
                        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
                            ...
                            <!-- 主題賽道篩選 (P1) -->
                            <div>
                                <select id="filter-track" class="tv-input w-full px-2.5 py-1.5 text-xs bg-slate-100 border border-slate-200 rounded-lg text-slate-800 font-medium focus:bg-white focus:border-orange-500 outline-none cursor-pointer">
                                    <option value="all">所有主題賽道 (Tracks)</option>
                                    ${trackOptions}
                                </select>
                            </div>
                        ...
                        <div class="flex flex-wrap items-center justify-between pt-1 text-xs text-slate-500 dark:text-slate-400 gap-2">
                            <div class="flex flex-wrap items-center gap-3">
                                ...
                                <div class="flex items-center gap-1.5">
                                    <span class="font-semibold text-slate-700 dark:text-slate-300">標籤:</span>
                                    <select id="filter-tag" class="tv-input px-2 py-1 bg-slate-50 border border-slate-200 rounded-md font-medium text-slate-700 focus:border-orange-500 outline-none cursor-pointer">
                            ...
                            <div class="flex items-center gap-4">
                                ...
                                <label class="flex items-center gap-1.5 cursor-pointer hover:text-slate-900 dark:hover:text-white transition-colors">
                                    <input type="checkbox" id="toggle-track" ${this._state.visibleColumns.track ? 'checked':''} class="rounded text-blue-600 focus:ring-blue-500">
                                    <span>主題賽道 (Track)</span>
                                </label>
                                <label class="flex items-center gap-1.5 cursor-pointer hover:text-slate-900 dark:hover:text-white transition-colors">
            ...
            const tableRows = this._state.filtered.map(item => {
                ...
                return `
                    <tr class="table-row hover:bg-slate-50 transition-colors border-b border-slate-100 dark:border-slate-800">
                        ...
                        ${this._state.visibleColumns.track ? `
                            <td class="p-3 whitespace-nowrap">
                                ${item.trackId ? `<span class="px-2 py-0.5 rounded text-[11px] font-bold border" style="background-color: ${item.trackHex}15; color: ${item.trackHex}; border-color: ${item.trackHex}35;">${item.trackName}</span>` : '<span class="text-slate-400">-</span>'}
                            </td>
                        ` : ''}
                        <td class="p-3 whitespace-nowrap">
        ...
        createStyles() {
            ...
            style.textContent = `
                ...
                /* ==========================================================================
                   Category Badges & Pills (Universal SSOT Taxonomy Styling)
                   ========================================================================== */
                .cat-badge {
                    display: inline-flex;
                    align-items: center;
                ...
                :host-context(html.dark) .cat-badge,
                :host-context(.dark) .cat-badge {
                ...
                :host-context(html.dark) .tag-pill,
                ...
                :host-context(html.dark) .keyword-pill,
                :host-context(.dark) .keyword-pill {
                    background-color: #181c27 !important;
                    color: #94a3b8 !important;
                    border-color: #2a2e39 !important;
                }
                :host-context(html.dark) .keyword-pill:hover,
                :host-context(.dark) .keyword-pill:hover {
                ...
                :host-context(html.dark) #count-badge {
                    background-color: rgba(37, 99, 235, 0.2) !important;
                    color: #93c5fd !important;
                    border-color: rgba(59, 130, 246, 0.3) !important;
                }

                :host-context(html.dark) #btn-reset-filters {
                    background-color: #1e222d !important;
                    color: #94a3b8 !important;
                    border-color: #2a2e39 !important;
                }
                :host-context(html.dark) #btn-reset-filters:hover {

...
(function(global) {
    ...
    class GlobalSearchWidget extends HTMLElement {
        constructor() {
            super();
            // closed mode does not assign this.shadowRoot; store the reference manually
        ...
        connectedCallback() {
            this.render();
            this.elements = {
                overlay: this._shadowRoot.querySelector('.search-overlay'),
                modal: this._shadowRoot.querySelector('.search-modal'),
                input: this._shadowRoot.querySelector('.search-input'),
                resultsContainer: this._shadowRoot.querySelector('.search-results')
            };
        ...
        open() {
            // Cancel any pending close-cleanup timer to avoid race condition
            if (this._closeTimer) {
            ...
            this._isOpen = true;
            // Clear previous search state before opening
            ...
            this.search(''); // Load default state
        }
        ...
        close() {
            ...
            // Store timer reference so open() can cancel it if user reopens quickly
        ...
        search(query) {
            ...
            if (global.CoreKnowledgeRepository && Array.isArray(global.CoreKnowledgeRepository.all)) {
                ...
                const matchedItems = items.filter(i => {
                    ...
                    url: `${prefix}post/crypto-knowledge.html?q=${encodeURIComponent(i.title)}`,
                    icon: '💡'
                }));
        ...
        render() {
            this._shadowRoot.innerHTML = this.getStyles() + `

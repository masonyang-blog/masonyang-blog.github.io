...
(function (global) {
    ...
    class ResponsiveTocComponent {
        ...
        init() {
            // 1. Locate Content with sequential fallback priority
            const selectors = (typeof this._config.contentSelector === 'string' && this._config.contentSelector.includes(','))
                ? this._config.contentSelector.split(',').map(s => s.trim())
        ...
        _renderMobile() {
            ...
            style.textContent = `
                ...
                /* Right Edge Handle Bar (Attached to right boundary) */
                .edge-handle-trigger {
                    position: fixed;
                    right: 0;
                    top: 50%;
                    ...
                    align-items: center;
                    gap: 0.35rem;
                    padding: 0.75rem 0.35rem 0.75rem 0.45rem;
                ...
                :host-context(html.dark) .drawer-panel { background: #0f172a; border-left: 1px solid #1e293b; }
            ...
            edgeHandleTrigger.innerHTML = `
                <svg viewBox="0 0 24 24"><path d="M4 6h16M4 12h16M4 18h7" stroke-linecap="round" stroke-linejoin="round"></path></svg>
                <span>目錄</span>
                <svg class="arrow-icon" viewBox="0 0 24 24"><polyline points="9 6 15 12 9 18" stroke-linecap="round" stroke-linejoin="round"></polyline></svg>
            `;
            ...
            // Gesture Swipe Listeners (Swipe Left from Right Edge to Open, Swipe Right to Close)
            let touchStartX = 0;
            let touchStartY = 0;
            ...
            document.addEventListener('touchend', (e) => {
                ...
                // 1. Edge Swipe Left to Open (Touch start within right edge 35px)
                if (!drawer.classList.contains('open')) {
                    if (windowWidth - touchStartX < 35 && deltaX < -45 && Math.abs(deltaY) < Math.abs(deltaX) * 0.85) {
                        openDrawer();
                    }
                } else {
                    // 2. Swipe Right to Close Drawer

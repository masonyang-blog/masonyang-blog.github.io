...
(function (global) {
    ...
    class HeaderComponent {
        constructor(hostElement) {
            ...
            // 賽道導航子項目 (12 大主題賽道 SSOT)
            const trackLinks = [
                { text: "總體經濟與信貸危機", href: `${prefix}post/archive.html?track=track-macro-credit` },
                { text: "黃金超級週期與儲備", href: `${prefix}post/archive.html?track=track-gold-supercycle` },
                { text: "AI 晶片與記憶體週期", href: `${prefix}post/archive.html?track=track-ai-semiconductor` },
                { text: "礦企轉型與電力競賽", href: `${prefix}post/archive.html?track=track-mining-ai-capex` },
                { text: "算力設施與 AI Agent", href: `${prefix}post/archive.html?track=track-compute-agent-energy` },
                { text: "美股估值與 SaaS 重定", href: `${prefix}post/archive.html?track=track-us-stocks-valuation` },
                { text: "比特幣指標與主權流動", href: `${prefix}post/archive.html?track=track-btc-sovereign-liquidity` },
                { text: "Web3 協議與 DeFAI", href: `${prefix}post/archive.html?track=track-web3-defai-regulation` },
                { text: "日圓套息與美債風暴", href: `${prefix}post/archive.html?track=track-jpy-carry-bonds` },
                { text: "能源大宗與地緣霸權", href: `${prefix}post/archive.html?track=track-energy-geopolitics` },
                { text: "政治週期與科技趨勢", href: `${prefix}post/archive.html?track=track-politics-tda-future` },
                { text: "投研方法論與心智模型", href: `${prefix}post/archive.html?track=track-methodology-research` }
            ];
            ...
            this._config = {
                ...
                navLinks: [
                    ...
                    },
                    {
                        ...
                        isDropdown: true,
                        children: [
                            { text: "全站文章試算表", href: `${prefix}post/articles-spreadsheet.html` },
                            { text: "AI 礦企核心數據對標總表", href: `${prefix}tools/ai-miners-sheet/ai-miners-core-metrics-benchmark.html` },
                            { text: "知識百科資料庫", href: `${prefix}post/crypto-knowledge.html` },
                            { text: "比特幣曼陀羅 (Mandala)", href: `${prefix}post/topic-btc-02-price-factors.html` }
                        ]
                    }
                ]
            };
        ...
        getPathPrefix() {
            ...
            // Depth-2 pages (e.g. /tools/topic-knowledge/, /tools/dca-simulator/)
            // These are 2 levels deep and need '../../' to reach root.
            if (path.includes('/' + 'tools/')) {
                return '../../';
            }

            // Depth-1 pages (e.g. /post/, /news/, /project/, /knowledge/, /doc/)
            if (
                path.includes('/' + 'post/') ||
            ...
            // Root level (index.html, pulse.html, etc.)
            return './';
        }
        ...
        createContent() {
            ...
            const navItems = this._config.navLinks.map(link => {
                if (link.isDropdown) {
                    const wideClass = link.isWide ? ' dropdown-wide' : '';
                    return `
        ...
        attachEvents() {
            ...
            // Search Logic: redirect to post/archive.html with ?q= query param
            const archiveUrl = `${this.getPathPrefix()}post/archive.html`;

            const goToSearch = (query) => {
                const url = query
                    ? `${archiveUrl}?q=${encodeURIComponent(query.trim())}`
                    : archiveUrl;
            ...
            // Global Cmd+K / Ctrl+K shortcut: focus the header search input
            ...
            this.elements.searchInputs.forEach(input => {
                // Remove readonly so users can type directly in the header search box
                input.removeAttribute('readonly');

                // Click: focus input (already focused by browser, no nav needed yet)
                // Enter key: navigate with typed query
                ...
                // Click on the search icon (parent .search-container) or the placeholder
                // If user clicks but hasn't typed anything, still navigate to archive
            ...
            // Headroom (Smart Header Hide on Scroll)
            let lastScrollY = window.scrollY;
            let ticking = false;
            const headerEl = this.shadowRoot.querySelector('header');
            
            window.addEventListener('scroll', () => {
                const isScrollHideDisabled = this._config.hideOnScroll === false || 
                    (this.hostElement && (this.hostElement.dataset.disableScrollHide === 'true' || this.hostElement.hasAttribute('data-disable-scroll-hide')));
                
                if (isScrollHideDisabled) {
            ...
            // Lightbox Lazy Load
            if (document.querySelector('.prose img:not(.no-lightbox)')) {
                const prefix = this.getPathPrefix();
                ...
                loadScript(`${prefix}assets/js/widget-lightbox.js`);
            }
        }

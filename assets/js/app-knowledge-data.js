...
(function (global) {
    ...
    class CoreKnowledgeRepository {
        ...
        /**
         * Get graph topology representation (Nodes & Edges) for Force-Directed Knowledge Graph
        ...
        getGraphTopology() {
            ...
            Object.entries(this._categories).forEach(([catId, catLabel]) => {
                const categoryNodeId = `cat-${catId}`;
                nodes.push({
                    id: categoryNodeId,
                    label: catLabel,
                    type: 'Category',
            ...
            // 2. Add Concept (Knowledge Card) Nodes
            _knowledgeData.forEach(item => {
                const conceptId = item.id;
                nodes.push({
                    id: conceptId,
                    label: item.title,
                    type: item.tag === 'onchain' ? 'Metric' : 'Concept',
                    category: item.tag || 'core',
                    desc: item.desc || '',
                ...
                // Category -> Concept Edge
                if (item.tag && this._categories[item.tag]) {
                    edges.push({
                        source: `cat-${item.tag}`,
                        target: conceptId,
                        relation: 'defines',
                ...
                // Article Link Edge (if relatedArticleId exists)
                if (item.relatedArticleId) {
                    const articleNodeId = `art-${item.relatedArticleId}`;
                    if (!nodeSet.has(articleNodeId)) {
                        nodes.push({
                            id: articleNodeId,
                            label: item.relatedArticleId,
                            type: 'Article',
                            category: item.tag || 'core',
                            weight: 10
                        });
                        nodeSet.add(articleNodeId);
                    }
                    edges.push({
            ...
            // 3. Add Explicit Directed Edges from _knowledgeLinks
            _knowledgeLinks.forEach(link => {
                if (nodeSet.has(link.from) && nodeSet.has(link.to)) {
                    edges.push({
                        source: link.from,
                        target: link.to,
                        relation: link.type || 'relatedTo',
                        label: link.label || '',
                        weight: 0.6
                    });
                }
        ...
        /**
         * Get 1st-degree neighbors for a specific node ID (for AI Sub-graph Insight)
         */
        getRelatedGraphNeighbors(nodeId) {
            const topology = this.getGraphTopology();
            const targetNode = topology.nodes.find(n => n.id === nodeId);
            if (!targetNode) return { node: null, neighbors: [], connectedEdges: [] };

            const connectedEdges = topology.edges.filter(e => e.source === nodeId || e.target === nodeId);
            const neighborIds = new Set(connectedEdges.map(e => e.source === nodeId ? e.target : e.source));
            const neighbors = topology.nodes.filter(n => neighborIds.has(n.id));

            return { node: targetNode, neighbors, connectedEdges };
        }
    }
